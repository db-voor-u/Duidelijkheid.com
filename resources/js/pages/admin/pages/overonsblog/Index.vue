...
const pageTitle = computed(() => {
    if (props.filters.type === 'innovatie') return 'Innovatie Beheer'
    if (props.filters.type === 'over_ons') return 'Over Ons Blogbeheer'
    return 'Over Ons & Innovatie Beheer'
})

const pageDesc = computed(() => {
    if (props.filters.type === 'innovatie') return 'Beheer hier de innovatie artikelen.'
    if (props.filters.type === 'over_ons') return 'Beheer hier de over ons artikelen.'
    return 'Beheer hier alle Over Ons en Innovatie gerelateerde artikelen.'
})

const listTitle = computed(() => {
    if (props.filters.type === 'innovatie') return 'Alle Innovatie Artikelen'
    if (props.filters.type === 'over_ons') return 'Alle Over Ons Artikelen'
    return 'Alle Artikelen'
})

function doSearch() {
    router.get(routes.admin.overonsBlog, { search: q.value, type: props.filters.type }, { preserveState: true, replace: true })
}

// FIX: Menu gebruikt nu slug en publicUrl
...
function getPublicUrl(row: PostRow) {
    if (row.category?.type === 'innovatie') {
        return `/innovatie/${row.slug}`
    }
    return `/over-ons-blog/${row.slug}`
}
...
// FIX: destroyPost gebruikt slug
function destroyPost(slug: string) {
    if (!confirm('Weet je zeker dat je deze Over Ons blog wilt verwijderen?')) return
    router.delete(adminRouteFns.overonsBlogDestroy(slug), {
        preserveScroll: true,
        onFinish: closeMenu
    })
}
    ...
    >
        <div class="space-y-6 p-6 lg:p-8">
            ...
            <Card class="border-0 shadow-lg">
                <CardHeader class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <CardTitle class="text-xl">{{ listTitle }}</CardTitle>
                    <div class="flex flex-wrap gap-2">
                        ...
                        <Link :href="routes.admin.overonsBlogCreate + (props.filters.type ? '?section=' + props.filters.type : '')">
                    ...
                    <table class="w-full text-sm">
                        ...
                        >
                            <td class="px-4 py-4 lg:px-6">
                                ...
                                <div class="text-xs text-muted-foreground">{{ getPublicUrl(row) }}</div>
                            </td>
                            <td class="px-4 py-4 lg:px-6">
                        ...
                        <tr v-if="!posts.data.length">
                            <td colspan="7" class="px-6 py-10 text-center text-muted-foreground">
                                Nog geen artikelen. Maak je eerste Over Ons blog aan!
                            </td>
                        </tr>
                        </tbody>
                    </table>
        ...
        <Teleport to="body">
            <div v-if="menu.open">
                ...
                >
                    <!-- FIX: Alle menu items gebruiken slug -->
                    <Link
                        :href="menu.publicUrl"
                        target="_blank"
                        rel="noopener"
                    ...
                    <Link
                        :href="adminRouteFns.overonsBlogEdit(menu.slug)"
                        class="flex w-full items-center gap-2 rounded-sm px-2 py-1.5 text-sm transition-colors hover:bg-accent"
                        @click="closeMenu"
                    >
